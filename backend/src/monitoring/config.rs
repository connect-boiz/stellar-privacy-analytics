    ...
    fn default() -> Self {
        Self {
            ...
            duration_buckets: vec![
                0.001, 0.005, 0.01, 0.05, 0.1, 0.5, 1.0, 2.5, 5.0, 10.0, 25.0, 50.0, 100.0, 250.0,
...
impl MonitoringConfig {
    ...
    pub fn from_env() -> Self {
        ...
        if let Ok(epsilon_threshold) = std::env::var("ALERT_EPSILON_THRESHOLD") {
            config.alerting.epsilon_consumption_threshold =
                epsilon_threshold.parse().unwrap_or(0.1);
        }
    ...
    pub fn validate(&self) -> Result<(), String> {
        ...
        if self.alerting.storage_failure_threshold < 0.0
