...
impl StellarAnalytics {
    ...
    pub fn initialize(env: Env, admin: Address) {
        ...
        // Emit a payload-bearing initialization event with a replay-detection
        // nonce (issue #412 WS5: every state-changing function emits one).
        let event_nonce = Self::next_event_nonce(&env);
        env.events().publish(
            (Symbol::new(&env, "analytics_initialized"),),
            (event_nonce, admin, env.ledger().timestamp()),
        );
    }
    ...
    pub fn request_analysis(
        ...
        // Generate request ID. A per-requester monotonic nonce is folded into
        // the hash so two requests with identical inputs within the same ledger
        // cannot collide (previously the ID was derived only from inputs +
        // ledger time/sequence, so a same-ledger duplicate silently overwrote
        // the first request while debiting the budget twice).
        ...
        let user_budget: i128 = Self::get_user_privacy_budget(&env, &requester);
        ...
        // Store the request (rejecting any residual ID collision instead of
        // overwriting the existing request)
        ...
        if requests.contains_key(request_id.clone()) {
            return Err(StellarAnalyticsError::InvalidRequestId);
        }
        ...
        // Update user privacy budget (checked; fail-closed on underflow)
        let new_budget = user_budget
            .checked_sub(DEFAULT_PRIVACY_BUDGET)
            .ok_or(StellarAnalyticsError::Overflow)?;
        Self::set_user_privacy_budget(&env, &requester, new_budget);
        ...
        let total_budget_used: i128 = env
            ...
            .unwrap_or(0);
        let new_total_budget_used = total_budget_used
            .checked_add(DEFAULT_PRIVACY_BUDGET)
            .ok_or(StellarAnalyticsError::Overflow)?;
        env.storage().instance().set(
            &Symbol::new(&env, "total_privacy_budget_used"),
            &new_total_budget_used,
        );
        ...
        // Emit payload-bearing event with a replay-detection nonce so indexers
        // can reconstruct request parameters (previously the payload was empty
        // `()` — events could not be used to rebuild state).
        let event_nonce = Self::next_event_nonce(&env);
        env.events().publish(
    ...
    pub fn complete_analysis(
        ...
        // Update active analyses count (checked; fail-closed on underflow)
        let active_analyses: u64 = env
            .storage()
            .instance()
            .get(&Symbol::new(&env, "active_analyses"))
            .unwrap_or(0);
        let new_active = active_analyses
            .checked_sub(1)
            .ok_or(StellarAnalyticsError::Overflow)?;
        env.storage()
            .instance()
            .set(&Symbol::new(&env, "active_analyses"), &new_active);

        // Refund unused privacy budget and update global counter (checked math)
        let refund = privacy_budget_for_refund
            .checked_sub(privacy_budget_used)
            .ok_or(StellarAnalyticsError::Overflow)?;
        if refund > 0 {
            let current_budget = Self::get_user_privacy_budget(&env, &requester_for_refund);
            let new_budget = current_budget
                .checked_add(refund)
                .ok_or(StellarAnalyticsError::Overflow)?;
            Self::set_user_privacy_budget(&env, &requester_for_refund, new_budget);
    ...
    pub fn cancel_analysis(
        ...
        // Refund privacy budget and update global counter (checked math)
        let current_budget = Self::get_user_privacy_budget(&env, &cancel_requester);
        let new_budget = current_budget
            .checked_add(cancel_budget)
            .ok_or(StellarAnalyticsError::Overflow)?;
        Self::set_user_privacy_budget(&env, &cancel_requester, new_budget);
    ...
    pub fn add_privacy_budget(
        ...
        let current_budget = Self::get_user_privacy_budget(&env, &user);
        let new_budget = current_budget
            .checked_add(amount)
            .ok_or(StellarAnalyticsError::Overflow)?;
        if new_budget > MAX_PRIVACY_BUDGET {
            return Err(StellarAnalyticsError::BudgetExceeded);
        }

        Self::set_user_privacy_budget(&env, &user, new_budget);

        // Emit payload-bearing event with nonce
        let event_nonce = Self::next_event_nonce(&env);
        env.events().publish(
            (Symbol::new(&env, "budget_added"), user.clone()),
            (event_nonce, amount, new_budget, env.ledger().timestamp()),
        );
    ...
    pub fn add_oracle(
        ...
        // Payload-bearing event: operator + nonce for replay detection
        let event_nonce = Self::next_event_nonce(&env);
        env.events().publish(
            (Symbol::new(&env, "oracle_added"), oracle.clone()),
            (event_nonce, caller.clone(), env.ledger().timestamp()),
        );
    ...
    pub fn remove_oracle(
        ...
        let event_nonce = Self::next_event_nonce(&env);
        env.events().publish(
            (Symbol::new(&env, "oracle_removed"), oracle.clone()),
            (event_nonce, caller.clone(), env.ledger().timestamp()),
        );
    ...
    /// Per-user privacy budget stored under `(Symbol("budget"), user)` instead
    /// of one giant `Map<Address, i128>` instance key — a single user's
    /// operation no longer pays for an O(N) rewrite of every user's budget
    /// (issue #412 WS3 gas-DoS hardening).
    fn get_user_privacy_budget(env: &Env, user: &Address) -> i128 {
        env.storage()
            .persistent()
            .get(&(Symbol::new(env, "budget"), user.clone()))
            .unwrap_or(0)
    }
    ...
    fn set_user_privacy_budget(env: &Env, user: &Address, budget: i128) {
        env.storage()
            .persistent()
            .set(&(Symbol::new(env, "budget"), user.clone()), &budget);
    }

    /// Monotonically increasing per-requester nonce folded into request IDs so
    /// identical same-ledger requests cannot collide (issue #412 WS2).
    ...
    /// Fail-closed ledger-consistency check (issue #412 WS5). After every
    /// request-lifecycle mutation the stored counters must match what the
    /// underlying `analysis_requests`/`analysis_results` maps imply:
    ///
    /// * `active_analyses` == count of requests with completed=false and
    ///   cancelled=false;
    /// * `total_privacy_budget_used` == Σ over requests of the retained budget
    ///   (full budget for pending, `privacy_budget_used` for completed, 0 for
    ///   cancelled).
    fn verify_state(env: &Env) -> Result<(), StellarAnalyticsError> {
        let requests: Map<BytesN<32>, AnalysisRequest> = env
            .storage()
            .instance()
            .get(&Symbol::new(env, "analysis_requests"))
        ...
        let mut expected_budget_used: i128 = 0;

        for (request_id, request) in requests.iter() {
            if request.cancelled {
                continue; // contributes 0 to retained budget
            }
            if request.completed {
                if let Some(result) = results.get(request_id) {
                    expected_budget_used = expected_budget_used
                        .checked_add(result.privacy_budget_used)
                        .ok_or(StellarAnalyticsError::Overflow)?;
                }
            } else {
                active_count += 1;
                expected_budget_used = expected_budget_used
                    .checked_add(request.privacy_budget)
                    .ok_or(StellarAnalyticsError::Overflow)?;
            }
        }

        let stored_active: u64 = env
            .storage()
            .instance()
            .get(&Symbol::new(env, "active_analyses"))
            .unwrap_or(0);
        ...
        if stored_active != active_count || stored_used != expected_budget_used {
            return Err(StellarAnalyticsError::StateInconsistent);
        }
        Ok(())
    }
    ...
    pub fn register_dataset(
        ...
        // The uploader must authorize the registration so datasets cannot be
        // registered under a victim's identity (previously the `uploader`
        // argument was spoofable and could pollute the ipfs_datasets map).
        ...
        let event_nonce = Self::next_event_nonce(&env);
        env.events().publish(
            (Symbol::new(&env, "dataset_registered"), uploader.clone()),
            (
                event_nonce,
                cid,
                dataset_hash_for_event,
                size_bytes,
                env.ledger().timestamp(),
            ),
        );
    ...
    pub fn pin_dataset(
        ...
        // Emit a payload-bearing event with a replay-detection nonce (issue
        // #412 WS5): pinning must be reconstructable by indexers.
        let event_nonce = Self::next_event_nonce(&env);
        env.events().publish(
            (Symbol::new(&env, "dataset_pinned"), cid.clone()),
            (event_nonce, caller, env.ledger().timestamp()),
        );
    ...
    pub fn create_dataset_version(
        ...
        // The uploader must authorize version creation (same spoofing
        // protection as register_dataset).
        uploader.require_auth();
...
mod test {
    ...
    /// WS2 acceptance: two `request_analysis` calls in the same ledger with
    /// identical inputs yield two distinct request IDs and debit the budget
    /// exactly twice (no overwrite). Previously the second call silently
    /// overwrote the first while charging the user twice.
    #[test]
    fn test_same_ledger_duplicate_request_ids_are_distinct() {
        let env = Env::default();
        env.mock_all_auths();
        let (_contract_id, client, admin, user, _oracle) = setup(&env);

        client.add_privacy_budget(&admin, &user, &(BUDGET * 2));
        let (cid, dataset_hash) = register_test_dataset(&env, &client, &user);

        let analysis_type = String::from_str(&env, "descriptive");
        let privacy_level = String::from_str(&env, "minimal");
        let id1 = client.request_analysis(
            &user,
            &dataset_hash,
            &cid.clone(),
            &analysis_type.clone(),
        ...
        // Budget debited exactly twice; two active analyses; both stored.
        let (_total, budget_used, active) = client.get_stats();
        assert_eq!(budget_used, BUDGET * 2);
        assert_eq!(active, 2);
        assert_eq!(client.get_analysis_request(&id1).request_id, id1);
        assert_eq!(client.get_analysis_request(&id2).request_id, id2);
    }

    /// WS2 acceptance: topping up at the MAX_PRIVACY_BUDGET ceiling fails
    /// closed (BudgetExceeded) instead of wrapping.
    #[test]
    fn test_add_privacy_budget_at_ceiling_rejected() {
        let env = Env::default();
        env.mock_all_auths();
        let (_contract_id, client, admin, user, _oracle) = setup(&env);

        // Top up to the ceiling exactly.
        client.add_privacy_budget(&admin, &user, &MAX_PRIVACY_BUDGET);

        // Any further top-up must be rejected (fail-closed, no overflow).
        let res = client.try_add_privacy_budget(&admin, &user, &1i128);
        assert_eq!(res, Err(Ok(StellarAnalyticsError::BudgetExceeded)));
    }

    /// WS5 acceptance: `verify_state` runs after every mutation and fails the
    /// transaction on violation. Deliberately corrupting a counter makes the
    /// next mutation return StateInconsistent (the whole transaction reverts).
    #[test]
    fn test_verify_state_detects_corrupted_counters() {
        let env = Env::default();
        env.mock_all_auths();
        let (contract_id, client, admin, user, oracle) = setup(&env);
        ...
        // Corrupt the total_privacy_budget_used counter directly. (Corrupting
        // active_analyses would trip the mutation's own checked_sub first, so
        // this counter isolates the verify_state path.)
        ...
        // The next mutation must fail closed with StateInconsistent.
        let res = client.try_cancel_analysis(&user, &request_id);
        assert_eq!(res, Err(Ok(StellarAnalyticsError::StateInconsistent)));
    }

    /// WS5 acceptance: state-changing events carry a monotonically increasing
    /// nonce (replay detection) and an indexer can reconstruct the get_stats()
    /// counters exactly from the event stream alone.
    #[test]
    fn test_events_reconstruct_stats_and_nonces_monotonic() {
        let env = Env::default();
        env.mock_all_auths();
        let (_contract_id, client, admin, user, oracle) = setup(&env);

        // Indexer-observed event stream, accumulated per transaction (the host
        // clears its event buffer at each invocation, so capture after every
        // call, then replay the whole stream).
        let mut stream: std::vec::Vec<(Address, Vec<Val>, Val)> = std::vec::Vec::new();
        let mut drain = |env: &Env| {
        ...
        let analysis_type = String::from_str(&env, "descriptive");
        let privacy_level = String::from_str(&env, "minimal");
        let r1 = client.request_analysis(
            &user,
            &dataset_hash.clone(),
            &cid.clone(),
            &analysis_type.clone(),
        ...
        drain(&env);
        let r2 = client.request_analysis(
            &user,
            &dataset_hash,
            &cid,
            &analysis_type.clone(),
        ...
        // Complete r1 with half the budget, cancel r2 for a full refund.
        let result_hash = BytesN::<32>::from_array(&env, &[3u8; 32]);
        let proofs: Vec<BytesN<32>> = Vec::new(&env);
        client.complete_analysis(&oracle, &r1, &result_hash, &(BUDGET / 2), &95u32, &proofs);
        drain(&env);
        client.cancel_analysis(&user, &r2);
        drain(&env);

        // Replay the accumulated stream like an indexer, decoding each payload
        // as a Vec<Val> (the tuple's wire encoding) with TryFromVal per field.
        let mut last_nonce: u64 = 0;
        let mut requested = 0u64;
        let mut completed = 0u64;
        let mut cancelled = 0u64;
        let mut budget_debited: i128 = 0;
        let mut budget_refunded: i128 = 0;

        for event in stream {
            let (_contract, topics, data) = event;
            let topic0: Symbol = Symbol::try_from_val(&env, &topics.get(0).unwrap()).unwrap();
            let payload: Vec<Val> = Vec::try_from_val(&env, &data).unwrap();
            let nonce: u64 = u64::try_from_val(&env, &payload.get(0).unwrap()).unwrap();
            assert!(
                nonce > last_nonce,
                "event nonces must increase monotonically across the stream"
            );
            last_nonce = nonce;

            if topic0 == Symbol::new(&env, "analysis_requested") {
                requested += 1;
                let budget: i128 = i128::try_from_val(&env, &payload.get(6).unwrap()).unwrap();
                budget_debited = budget_debited.checked_add(budget).unwrap();
            } else if topic0 == Symbol::new(&env, "analysis_completed") {
                completed += 1;
                let used: i128 = i128::try_from_val(&env, &payload.get(2).unwrap()).unwrap();
                budget_refunded = budget_refunded.checked_add(BUDGET - used).unwrap();
            } else if topic0 == Symbol::new(&env, "analysis_cancelled") {
                cancelled += 1;
                budget_refunded = budget_refunded.checked_add(BUDGET).unwrap();
            }
        }

        // Reconstructed counters must match get_stats() exactly.
        let (total, used, active) = client.get_stats();
        assert_eq!(total, requested);
        assert_eq!(used, budget_debited - budget_refunded);
        assert_eq!(active, requested - completed - cancelled);
    }
}
