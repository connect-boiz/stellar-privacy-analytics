...
impl MultiSigAdmin {
    ...
    pub fn initialize(env: Env, owners: Vec<Address>, threshold: u32) -> Result<(), MultiSigError> {
        ...
        // Every owner must authorize their own inclusion. Without host-level
        // auth on initialization the first caller to front-run deployment
        // could register themselves as the sole owner with threshold 1 and
        // permanently own the multisig. Requiring each owner's signature makes
        // a front-run initialization that lists victims as owners impossible.
    ...
    pub fn add_owner(env: Env, new_owner: Address, caller: Address) -> Result<(), MultiSigError> {
        // Host-level auth first: without it any attacker could pass an owner's
        // address as `caller` and the is_owner() equality check would pass.
        caller.require_auth();
    ...
    pub fn remove_owner(
        ...
        // Host-level auth first (spoofing prevention, see add_owner).
        caller.require_auth();
    ...
    pub fn submit_transaction(
        ...
        // Increment nonce (checked; fail-closed on overflow)
        let new_nonce = nonce.checked_add(1).ok_or(MultiSigError::Overflow)?;
        env.storage()
            .instance()
            .set(&String::from_str(&env, NONCE_KEY), &new_nonce);
